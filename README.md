...
한 문제가 발생하기 때문에 두 라이브러리의 역할을 분리, 각 스텝은 가벼운 폼 상태를 갖고, 전역 상태는 Zustand가 보장
...
| 단계                    | AI 활용 내용                                                                       | 직접 판단하고 구현한 부분                                                                                         |
| :---------------------- | :--------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------- |
| **목업 데이터 생성**    | 강의 목록 더미 데이터 생성 요청                                                    | UI 테스트에 필요한 엣지 케이스(정원 마감 임박, 마감 등) 데이터 선별 및 구조화                                     |
| **상태 관리 (Zustand)** | Zustand의 기본 사용법 및 보일러플레이트 코드 참고                                  | 프로젝트 흐름에 맞춰 `persist` 미들웨어 적용 및 로컬 스토리지 저장 예외 처리(`partialize`) 직접 구현              |
| **코드 리팩토링 검토**  | 컴포넌트 분리 후 누락된 Props나 로직이 없는지 코드 리뷰 요청                       | '오케스트레이터 - 하위 UI 컴포넌트'로 구조를 나누는 최종 아키텍처 결정                                            |
| **에러 디버깅**         | `react-hook-form` 중첩 객체 타입 에러 등 복잡한 타입스크립트 가능한 에러 원인 파악 | AI가 편의를 위해 제안한 `any` 사용을 배제하고, 제네릭(`T`)을 활용해 완벽한 타입 안정성 확보                       |
| **프로덕션 레벨 설계**  | 프로덕션 환경에서의 폼 상태 관리 및 컴포넌트 아키텍처 설계 질문                    | 조언을 바탕으로 URL 기반 라우팅, 스텝 전환과 같이 실제 UX에 맞는 형태로 커스터마이징 적용                         |
| **README 문서화**       | 프로젝트 구조 설명, 요구사항 해석 등 기본 문서 템플릿 작성 지원                    | 설계 결정 근거, 미구현 사항에 대한 기술적 판단과 같은 핵심 내용은 직접 작성 및 생성된 내용 전체 검토 후 오류 수정 |

AI는 프로젝트 초기 구조 설계와 에러 해결 과정에서 빠르고 유용한 해결책을 제시했지만 다음과 같은 한계 존재:
